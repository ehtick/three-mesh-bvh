import{H as e,Qn as t,Wr as n,Y as r,b as i,dr as a,f as o,o as s,p as c,rn as l,un as u,zt as d}from"./ExtendedTriangle-cvTTtm2k.js";import{t as f}from"./Pass-BZUemdGI.js";import{t as p}from"./lil-gui.module.min-CCk8J1jY.js";import{t as m}from"./GLTFLoader-k9Ro4ZOw.js";import{t as h}from"./OrbitControls-CZExYGpc.js";import{t as g}from"./meshopt_decoder.module-DDLmvpjg.js";import{t as _}from"./stats.module-BDErWxYO.js";import{C as v,_ as y,b,c as x,f as S,i as C,m as w,o as T,p as E,s as D,t as O,u as k,x as A}from"./BVHComputeData--SEhHMdr.js";var j=class extends b{constructor(e){super();let t={surface:S(0),normalStep:S(new n),projectionInverse:S(new d),sdfTransformInverse:S(new d),sdfTransform:S(new d),uv:w(E()),sdf_sampler:D(e),sdf:k(e)},r=y(`
			fn raymarch(
				surface: f32,
		...
		`)]);this.fragmentNode=r(t);let i={position:T},a=y(`

			fn noop(position: vec4f) -> vec4f {
				return position;
			}

		`);this.vertexNode=a(i)}},M=class extends b{constructor(e){super();let t=y(`
		...
		`);this.vertexNode=a(i)}},N=[4,4,4],P={resolution:75,margin:.2,regenerate:()=>Z(),mode:`raymarching`,layer:0,surface:.1},F,I,L,R,z,B,V,H,U,W,G,K,q,J=new d;Y().then(Q);async function Y(){if(!await navigator.gpu.requestAdapter())throw document.body.appendChild($()),Error(`No WebGPU support`);V=document.getElementById(`output`),F=new v,F.setPixelRatio(window.devicePixelRatio),F.setSize(window.innerWidth,window.innerHeight),F.setClearColor(0,0),document.body.appendChild(F.domElement),await F.init(),L=new a;let t=new e(16777215,3);t.position.set(1,1,1),L.add(t),L.add(new s(16777215,.2)),I=new l(75,window.innerWidth/window.innerHeight,.1,50),I.position.set(1,1,2),I.far=100,I.updateProjectionMatrix(),B=new c(new o),L.add(B),new h(I,F.domElement),z=new _,document.body.appendChild(z.dom),U=(await new m().setMeshoptDecoder(g).loadAsync(`https://raw.githubusercontent.com/gkjohnson/3d-demo-data/main/models/stanford-bunny/bunny.glb`)).scene,U.updateMatrixWorld(!0),new o().setFromObject(U).getCenter(U.position).multiplyScalar(-1),U.updateMatrixWorld(!0),L.add(U),K=new O(U,{attributes:{position:`vec4f`}}),K.update();let n={matrix:S(new d),dim:S(0),globalId:C,output:x(H)};q=y(`
	...
	`,[K.fns.closestPointToPoint,K.fns.raycastFirstHit])(n).computeKernel(N),X(),window.addEventListener(`resize`,function(){I.aspect=window.innerWidth/window.innerHeight,I.updateProjectionMatrix(),F.setSize(window.innerWidth,window.innerHeight)},!1),Z(),W=new f(new M(H)),G=new f(new j(H))}function X(){R&&R.destroy(),P.layer=Math.min(P.resolution,P.layer),R=new p;let e=R.addFolder(`generation`);e.add(P,`resolution`,10,200,1),e.add(P,`margin`,0,1),e.add(P,`regenerate`);let t=R.addFolder(`display`);t.add(P,`mode`,[`geometry`,`raymarching`,`layer`,`grid layers`]).onChange(()=>{X()}),P.mode===`layer`&&t.add(P,`layer`,0,P.resolution-1,1),P.mode===`raymarching`&&t.add(P,`surface`,-.2,.5)}function Z(){let e=P.resolution,a=new d,s=new n,c=new u,l=new n,f=new o().setFromObject(U);if(f.getCenter(s),l.subVectors(f.max,f.min),l.x+=2*P.margin,l.y+=2*P.margin,l.z+=2*P.margin,a.compose(s,c,l),J.copy(a).invert(),B.box.copy(f),B.box.min.x-=P.margin,B.box.min.y-=P.margin,B.box.min.z-=P.margin,B.box.max.x+=P.margin,B.box.max.y+=P.margin,B.box.max.z+=P.margin,H&&H.dispose(),H=new A(e,e,e),H.format=t,H.type=r,H.generateMipmaps=!1,H.needsUpdate=!0,H.wrapR=i,H.wrapS=i,H.wrapT=i,q&&(q.computeNode.parameters.output.value=H),W){let e=W.material;e.fragmentNode.parameters.sdf.value=H,e.fragmentNode.parameters.sdf_sampler.node.value=H}if(G){let e=G.material;e.fragmentNode.parameters.sdf.value=H,e.fragmentNode.parameters.sdf_sampler.node.value=H}let p=window.performance.now();q.computeNode.parameters.matrix.value.copy(a),q.computeNode.parameters.dim.value=e;let m=[Math.ceil(e/N[0]),Math.ceil(e/N[1]),Math.ceil(e/N[2])];F.compute(q,m),F.backend.device!==null&&F.backend.device.queue.onSubmittedWorkDone().then(()=>{let e=window.performance.now()-p;V.innerText=`${e.toFixed(2)}ms`}),X()}function Q(){if(z.update(),requestAnimationFrame(Q),H){if(P.mode===`geometry`)F.render(L,I);else if(P.mode===`layer`||P.mode===`grid layers`){let e=W.material;e.fragmentNode.parameters.layer.value=P.layer,e.fragmentNode.parameters.grid_mode.value=P.mode===`grid layers`,W.render(F)}else if(P.mode===`raymarching`){I.updateMatrixWorld(),U.updateMatrixWorld();let e=G.material;e.fragmentNode.parameters.surface.value=P.surface,e.fragmentNode.parameters.normalStep.value.set(1,1,1).divideScalar(H.width),e.fragmentNode.parameters.projectionInverse.value.copy(I.projectionMatrixInverse);let t=new d().copy(J).multiply(I.matrixWorld);e.fragmentNode.parameters.sdfTransformInverse.value.copy(t),t.invert(),e.fragmentNode.parameters.sdfTransform.value.copy(t),G.render(F)}}}function $(){let e=document.createElement(`div`);return e.id=`webgpumessage`,e.style.fontFamily=`monospace`,e.style.fontSize=`13px`,e.style.fontWeight=`normal`,e.style.textAlign=`center`,e.style.background=`#fff`,e.style.color=`#000`,e.style.padding=`1.5em`,e.style.maxWidth=`400px`,e.style.margin=`5em auto 0`,e.innerHTML=`Your browser does not support <a href="https://gpuweb.github.io/gpuweb/" style="color:blue">WebGPU</a> yet`,e}
//# sourceMappingURL=webgpu_sdfGeneration-DYN-oOpJ.js.map