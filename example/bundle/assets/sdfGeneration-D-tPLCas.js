import{n as e}from"./chunk-Bh1tDfsg.js";import{Bt as t,Et as n,H as r,M as i,Qn as a,Wr as o,Y as s,Yn as c,Yr as l,dr as u,f as d,fr as f,i as p,nt as m,o as h,p as g,qr as _,qt as v,rn as y,un as b,zt as x}from"./ExtendedTriangle-cvTTtm2k.js";import{t as S}from"./Pass-BZUemdGI.js";import{t as C}from"./MeshBVHUniformStruct-CsEKwaR-.js";import{t as w}from"./StaticGeometryGenerator-PHtdPgmL.js";import{n as T,r as E,t as D}from"./bvh_struct_definitions.glsl-D-uzwxra.js";import{t as O}from"./stats.min-CnMmk804.js";import{t as ee}from"./lil-gui.module.min-CCk8J1jY.js";import{t as te}from"./GenerateMeshBVHWorker-CTn10ZYk.js";import{t as k}from"./GLTFLoader-k9Ro4ZOw.js";import{t as A}from"./OrbitControls-CZExYGpc.js";import{t as j}from"./meshopt_decoder.module-DDLmvpjg.js";var M=`
...
`,N=e(O(),1),ne=class extends f{constructor(e){super({defines:{USE_SHADER_RAYCAST:+!!window.location.hash.includes(`USE_SHADER_RAYCAST`)},uniforms:{matrix:{value:new x},zValue:{value:0},bvh:{value:new C}},vertexShader:`
			...
			`}),this.setValues(e)}},re=class extends f{constructor(e){super({defines:{DISPLAY_GRID:0},uniforms:{sdfTex:{value:null},layer:{value:0},layers:{value:0}},vertexShader:`
			...
			`}),this.setValues(e)}},ie=class extends f{constructor(e){super({defines:{MAX_STEPS:500,SURFACE_EPSILON:.001},uniforms:{surface:{value:0},sdfTex:{value:null},normalStep:{value:new o},projectionInverse:{value:new x},sdfTransformInverse:{value:new x}},vertexShader:`
			...
			`}),this.setValues(e)}},P={gpuGeneration:!0,resolution:75,margin:.2,regenerate:()=>Q(),mode:`raymarching`,layer:0,surface:.1},F,I,L,R,z,B,V,H,U,W,G,K,q,J,Y,X=new x;ae(),$();function ae(){V=document.getElementById(`output`),F=new p({antialias:!0}),F.setPixelRatio(window.devicePixelRatio),F.setSize(window.innerWidth,window.innerHeight),F.setClearColor(0,0),document.body.appendChild(F.domElement),L=new u;let e=new r(16777215,1);e.position.set(1,1,1),L.add(e),L.add(new h(16777215,.2)),I=new y(75,window.innerWidth/window.innerHeight,.1,50),I.position.set(1,1,2),I.far=100,I.updateProjectionMatrix(),B=new g(new d),L.add(B),new A(I,F.domElement),z=new N.default,document.body.appendChild(z.dom),K=new S(new ne),q=new S(new re),J=new S(new ie),Y=new te,new k().setMeshoptDecoder(j).loadAsync(`https://raw.githubusercontent.com/gkjohnson/3d-demo-data/main/models/stanford-bunny/bunny.glb`).then(e=>{e.scene.updateMatrixWorld(!0);let t=new w(e.scene);return t.attributes=[`position`,`normal`],t.useGroups=!1,U=t.generate().center(),Y.generate(U,{maxLeafSize:1})}).then(e=>{H=e,G=new t(U,new v),L.add(G),Q()}),Z(),window.addEventListener(`resize`,function(){I.aspect=window.innerWidth/window.innerHeight,I.updateProjectionMatrix(),F.setSize(window.innerWidth,window.innerHeight)},!1)}function Z(){R&&R.destroy(),P.layer=Math.min(P.resolution,P.layer),R=new ee;let e=R.addFolder(`generation`);e.add(P,`gpuGeneration`),e.add(P,`resolution`,10,200,1),e.add(P,`margin`,0,1),e.add(P,`regenerate`);let t=R.addFolder(`display`);t.add(P,`mode`,[`geometry`,`raymarching`,`layer`,`grid layers`]).onChange(()=>{Z()}),P.mode===`layer`&&t.add(P,`layer`,0,P.resolution,1),P.mode===`raymarching`&&t.add(P,`surface`,-.2,.5)}function Q(){let e=P.resolution,t=new x,r=new o,u=new b,d=new o;U.boundingBox.getCenter(r),d.subVectors(U.boundingBox.max,U.boundingBox.min),d.x+=2*P.margin,d.y+=2*P.margin,d.z+=2*P.margin,t.compose(r,u,d),X.copy(t).invert(),B.box.copy(U.boundingBox),B.box.min.x-=P.margin,B.box.min.y-=P.margin,B.box.min.z-=P.margin,B.box.max.x+=P.margin,B.box.max.y+=P.margin,B.box.max.z+=P.margin,W&&W.dispose();let f=1/e,p=.5*f,h=window.performance.now();if(P.gpuGeneration){let r=F.extensions.get(`OES_texture_float_linear`);W=new _(e,e,e),W.texture.format=a,W.texture.type=r?s:m,W.texture.minFilter=n,W.texture.magFilter=n,F.initRenderTarget(W),K.material.uniforms.bvh.value.updateFrom(H),K.material.uniforms.matrix.value.copy(t);let i=new o,c=new l(e,e);c.texture.format=a,c.texture.type=r?s:m;for(let t=0;t<e;t++)K.material.uniforms.zValue.value=t*f+p,F.setRenderTarget(c),K.render(F),i.z=t,F.copyTextureToTexture(c.texture,W.texture,null,i);F.readRenderTargetPixels(c,0,0,1,1,new Float32Array(4)),F.setRenderTarget(null),c.dispose()}else{W=new i(new Float32Array(e**3),e,e,e),W.format=a,W.type=s,W.minFilter=n,W.magFilter=n,W.needsUpdate=!0;let r=new o,l=new c,u={};for(let n=0;n<e;n++)for(let i=0;i<e;i++)for(let a=0;a<e;a++){r.set(p+n*f-.5,p+i*f-.5,p+a*f-.5).applyMatrix4(t);let o=n+i*e+a*e*e,s=H.closestPointToPoint(r,u).distance;l.origin.copy(r),l.direction.set(0,0,1);let c=H.raycastFirst(l,2),d=c&&c.face.normal.dot(l.direction)>0;W.image.data[o]=d?-s:s}}let g=window.performance.now()-h;V.innerText=`${g.toFixed(2)}ms`,Z()}function $(){if(z.update(),requestAnimationFrame($),W){if(P.mode===`geometry`)F.render(L,I);else if(P.mode===`layer`||P.mode===`grid layers`){let e,t=q.material;W.isData3DTexture?(t.uniforms.layer.value=P.layer/W.image.width,t.uniforms.sdfTex.value=W,e=W):(t.uniforms.layer.value=P.layer/W.width,t.uniforms.sdfTex.value=W.texture,e=W.texture),t.uniforms.layers.value=e.image.width;let n=P.mode===`layer`?0:1;n!==t.defines.DISPLAY_GRID&&(t.defines.DISPLAY_GRID=n,t.needsUpdate=!0),q.render(F)}else if(P.mode===`raymarching`){I.updateMatrixWorld(),G.updateMatrixWorld();let e;e=W.isData3DTexture?W:W.texture;let{width:t,depth:n,height:r}=e.image;J.material.uniforms.sdfTex.value=e,J.material.uniforms.normalStep.value.set(1/t,1/r,1/n),J.material.uniforms.surface.value=P.surface,J.material.uniforms.projectionInverse.value.copy(I.projectionMatrixInverse),J.material.uniforms.sdfTransformInverse.value.copy(G.matrixWorld).invert().premultiply(X).multiply(I.matrixWorld),J.render(F)}}}
//# sourceMappingURL=sdfGeneration-D-tPLCas.js.map